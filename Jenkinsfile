...
def ecrUrl="183295413020.dkr.ecr.ap-northeast-2.amazonaws.com"
def repository="my-app-image1119"
...
stage('Build Docker Image & Push to AWS ECR') {
            ...
            // withAWS를 통해 리전과 계정의 access, secret 키를 가져옴
                withAWS(region: "${region}", credentials: "aws-key") {
                    // AWS에 접속해서 ECR을 사용해야 함, 젠킨스에는 aws-cli를 설치하지 않았음
                    // aws-cli 없이도 접속을 할 수 있게 도와주는 라이브러리 설치
                    // helper가 나 대신 aws에 로그인을 진행 그리고 그 인증 정보를 json으로 만들어서
                    // docker에게 세팅해 줌 - > docker가 ecr에 push가 가능해짐
                    sh """
